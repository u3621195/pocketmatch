...
- Fixed iPhone/iPad carousel not scrolling by applying native carousel CSS directly when `.native-carousel` is active.
- Avoided cancelling mobile pointerup events so iOS Safari can perform horizontal momentum scrolling normally.
- Preserved desktop controlled carousel with arrows and all gameplay logic.


## v1.3.16

- Fixed the iPhone/iPad startup carousel scroll issue.
- Removed native carousel parent overflow clipping that could block iOS scrolling.
- Replaced `scrollIntoView({ inline: "center" })` with direct `scrollLeft`/`scrollTo` centering for better iOS Safari reliability.
- Isolated desktop transform-based carousel CSS so it does not interfere with native mobile/tablet scrolling.
- Improved desktop controlled-carousel card spacing so scaled side cards have more consistent visual gaps.
